package com.ks.flowerrecognition.database
...
class Database(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VER) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val CREATE_TABLE_QUERY =
            "CREATE TABLE $TABLE_NAME ($COL_ID INTEGER PRIMARY KEY, $COL_NAME VARCHAR, $COL_DESC TEXT, $COL_PHOTO VARCHAR)"
    ...
    fun addFlower(flower: Flower) {
        ...
        values.put(
            COL_DESC, flower.desc
        )
        values.put(COL_PHOTO, flower.image)
    ...
    fun updateFlower(flower: Flower): Int {
        ...
        values.put(COL_ID, flower.id)
        values.put(COL_NAME, flower.name)
        values.put(COL_DESC, flower.desc)
        values.put(COL_PHOTO, flower.image)
